...
export default function AdminComplaintsPage() {
  ...
  if (loading)
    return <p className="text-center mt-20 text-slate-600">Loading complaints…</p>;
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-sky-100 via-blue-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800 px-4 py-10">
      <section className="max-w-7xl mx-auto bg-white/70 dark:bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-xl p-8 animate-float">
        ...
        {complaints.length === 0 ? (
          <p className="text-center text-slate-500">No complaints available.</p>
        ) : (
          <div className="overflow-x-auto rounded-2xl shadow">
            <table className="min-w-full bg-white/90 dark:bg-slate-900">
              <thead className="bg-slate-100 dark:bg-slate-800">
                <tr>
                  {["Title", "Student", "Department", "Status", "Created", "Action"].map((h) => (
                    <th key={h} className="py-3 px-4 text-left text-sm font-semibold text-slate-700 dark:text-slate-300">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {complaints.map((c) => (
                  <tr key={c.id} className="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                    <td className="py-3 px-4">{c.title}</td>
                    <td className="py-3 px-4">{c.student_id}</td>
